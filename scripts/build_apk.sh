...
#
# Delegates to dev.sh's `build` task rather than reimplementing the Gradle
# invocation here -- dev.sh is the single place that knows how to build this
# repo (see the project's dev-script conventions), so there's exactly one
# build implementation to keep in sync as the Gradle setup evolves. This
# script is just a convenience entry point, matching coding-adventure's
# build_apk.sh.
set -e
